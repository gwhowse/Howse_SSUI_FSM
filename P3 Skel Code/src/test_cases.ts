...
function test3() {
	let fsmInt = new FSMInteractor(undefined, 450,0);
	root.addChild(fsmInt);
...
// Custom Interactor: Moo Deng Meter!
// User is able to rate how their feeling on a scale of Moo Dengs (1-6) that increase in chaoticness
// Clicking in the different regions to select is supported as well as dragging the slider across regions.
function test4() {
	let fsmInt = new FSMInteractor(undefined, 0,350);
	root.addChild(fsmInt);
	fsmInt.startLoadFromJson("./fsm_json/moodeng.json");
}