...
export class FSM {
    ...
    protected _finalize() : void {
        ...
        // **** YOUR CODE HERE ****
        // let's just make one call to access this.states and this.regions
        const states : readonly State[] = this.states;
        const regions : readonly Region[] = this.regions;

        // for every state, and every transition inside of every state:
        for (let state of states) {
            for (let transition of state.transitions) {
                // bind the target
                transition.bindTarget(states);

                // bind the region for each onEvent
                transition.onEvent.bindRegion(regions);

                // bind the regions for every action
                for (let action of transition.actions) {
                    action.bindRegion(regions);
                }
            }
        }
        ...
        // **** YOUR CODE HERE ****
        // if we actually have states, set start and current state to our first state
        if (states.length > 0) {
            this._startState = states[0];
        ...
        // **** YOUR CODE HERE ****
        // this FSM should be the parent of every region
    ...
    public reset() {
        ...
        // **** YOUR CODE HERE ****
        // current state is now our start state
    ...
    public actOnEvent(evtType : EventType, reg? : Region) {
        ...
        // **** YOUR CODE HERE ****
        // Go through our transitions, taking the first one that we have a match for.
        for (let trans of this.currentState.transitions) {
            if (trans.match(evtType, reg)) {
                ...
                // move FSM to our new target state
                this._currentState = trans.target;

                // bail out so that we don't double dip with this event
                return;
            }
        }
