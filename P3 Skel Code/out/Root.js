...
export class Root {
    ...
    _redraw() {
        ...
        const ctx = this.canvasContext; //this is my code too, just storing this so that we don't have to keep calling the getter
        try {
            // **** YOUR CODE HERE ****
            ctx.clearRect(0, 0, this.owningCanvas.width, this.owningCanvas.height);
            this._children.forEach((child) => {
                ctx.save();
                // move to our child's local coordinates
